...
trait Jobable
{
    ...
    private function getJobDetails($job)
    {
        if (auth()->user()) {
            ...
            $job_details = $job->load(['benefits:id,name','tags:id,name','role:id,name', 'company.user' => function ($q) {
    ...
    public function jobTagsInsert($tags, $job){
        if ($tags) {
            ...
            foreach ($tags as $tag) {
                $taggable = Tag::where('id', $tag)->orWhere('name', $tag)->first();

                if (!$taggable) {
                    $select_tag = Tag::create(['name' => $tag]);
                    array_push($tagsArray, $select_tag->id);
                }else{
    ...
    {
        return Tag::withCount('tags')->latest('tags_count')->get()->take(10);
        // return Tag::join('job_tag', 'job_tag.tag_id', '=', 'tags.id')
        //     ->groupBy('tags.id')
